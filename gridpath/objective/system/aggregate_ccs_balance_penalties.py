...
def add_model_components(m, d, scenario_directory, subproblem, stage):
    ...
    def total_ccs_penalty_costs_rule(mod):
        return sum((mod.Unserved_CCS_Expression[z, tmp]
                    * mod.unserved_ccs_penalty_per_tonne[z] +
                    mod.Over_CCS_Expression[z, tmp]
                    * mod.over_ccs_penalty_per_tonne[z])
                   * mod.hrs_in_tmp[tmp]
                   * mod.tmp_weight[tmp]
                   * mod.number_years_represented[mod.period[tmp]]
                   * mod.discount_factor[mod.period[tmp]]
                   for z in mod.LOAD_ZONES for tmp in mod.TMPS)
    m.Total_CCS_Balance_Penalty_Costs = Expression(
        rule=total_ccs_penalty_costs_rule)
...
def record_dynamic_components(dynamic_components):
    ...
    getattr(dynamic_components, cost_components).append(
        "Total_CCS_Balance_Penalty_Costs"
    )
