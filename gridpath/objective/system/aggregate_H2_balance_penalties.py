...
def add_model_components(m, d, scenario_directory, subproblem, stage):
    ...
    def total_H2_penalty_costs_rule(mod):
        return sum((mod.Unserved_H2_Expression[z, tmp]
                    * mod.unserved_H2_penalty_per_mw[z] +
                    mod.Over_H2_Expression[z, tmp]
                    * mod.over_H2_penalty_per_mw[z])
                   * mod.hrs_in_tmp[tmp]
                   * mod.tmp_weight[tmp]
                   * mod.number_years_represented[mod.period[tmp]]
                   * mod.discount_factor[mod.period[tmp]]
                   for z in mod.LOAD_ZONES for tmp in mod.TMPS)
    m.Total_H2_Balance_Penalty_Costs = Expression(
        rule=total_H2_penalty_costs_rule)
...
def record_dynamic_components(dynamic_components):
    ...
    getattr(dynamic_components, cost_components).append(
        "Total_H2_Balance_Penalty_Costs"
    )
