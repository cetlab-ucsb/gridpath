...
def add_model_components(m, d, scenario_directory, subproblem, stage):
    ...
    m.allow_over_H2 = Param(m.LOAD_ZONES, within=Boolean)
    m.over_H2_penalty_per_mw = \
        Param(m.LOAD_ZONES, within=NonNegativeReals)
    m.allow_unserved_H2 = Param(m.LOAD_ZONES, within=Boolean)
    m.unserved_H2_penalty_per_mw = \
        Param(m.LOAD_ZONES, within=NonNegativeReals)

    m.allow_over_ccs = Param(m.LOAD_ZONES, within=Boolean)
    m.over_ccs_penalty_per_tonne = \
        Param(m.LOAD_ZONES, within=NonNegativeReals)
    m.allow_unserved_ccs = Param(m.LOAD_ZONES, within=Boolean)
    m.unserved_ccs_penalty_per_tonne = \
        Param(m.LOAD_ZONES, within=NonNegativeReals)
...
def get_inputs_from_database(scenario_id, subscenarios, subproblem, stage, conn):
    ...
    load_zones = c.execute("""
        ...
        allow_over_H2, over_H2_penalty_per_mw, 
        allow_unserved_H2, unserved_H2_penalty_per_mw,
        allow_over_ccs, over_ccs_penalty_per_tonne, 
        allow_unserved_ccs, unserved_ccs_penalty_per_tonne
