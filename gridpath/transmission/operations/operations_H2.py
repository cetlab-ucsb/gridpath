...
def export_results(scenario_directory, subproblem, stage, m, d):
    ...
    with open(os.path.join(scenario_directory, str(subproblem), str(stage), "results",
                           "H2_operations.csv"), "w", newline="") as \
            tx_op_results_file:
        ...
        for (l, tmp) in m.TX_OPR_TMPS:
            if m.tx_operational_type[l] == "H2_simple":
                writer.writerow([
                    l,
                    m.load_zone_from[l],
                    m.load_zone_to[l],
...
def import_results_into_database(
    ...
    setup_results_import(
        conn=db, cursor=c,
        table="results_H2_operations",
        scenario_id=scenario_id, subproblem=subproblem, stage=stage
    )

    # Load results into the temporary table
    results = []
    with open(os.path.join(results_directory, "H2_operations.csv"),
              "r") as tx_op_file:
    ...
    insert_temp_sql = """
        INSERT INTO temp_results_H2_operations{}
        (scenario_id, transmission_line, period, subproblem_id, 
        stage_id, timepoint, timepoint_weight, 
        number_of_hours_in_timepoint,
        load_zone_from, load_zone_to, H2_flow_mw,
        H2_losses_lz_from, H2_losses_lz_to)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?);
        """.format(scenario_id)
    ...
    insert_sql = """
        INSERT INTO results_H2_operations
        ...
        load_zone_from, load_zone_to, H2_flow_mw,
        H2_losses_lz_from, H2_losses_lz_to
        FROM temp_results_H2_operations{}
...
def process_results(db, c, scenario_id, subscenarios, quiet):
    ...
    """
    if not quiet:
        print("aggregate H2 imports exports")

    # Delete old results
    del_sql = """
        DELETE FROM results_H2_imports_exports_agg
        WHERE scenario_id = ?
        """
    ...
    agg_sql = """
        INSERT INTO results_H2_imports_exports_agg
        ...
        load_zone_to AS load_zone, spinup_or_lookahead,
        SUM(H2_flow_mw * timepoint_weight * 
