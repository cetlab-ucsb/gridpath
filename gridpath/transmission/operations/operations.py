...
def add_model_components(m, d, scenario_directory, subproblem, stage):
    ...
    def transmit_power_rule(mod, tx, tmp):
        ...
        if hasattr (imported_tx_operational_modules[tx_op_type],
                    'transmit_power_rule'):
            return imported_tx_operational_modules[tx_op_type].\
                transmit_power_rule(mod, tx, tmp)
        else:
            return tx_type_init.transmit_power_rule(mod, tx, tmp)
    ...
    def transmit_power_losses_lz_from_rule(mod, tx, tmp):
        ...
        if hasattr (imported_tx_operational_modules[tx_op_type],
            ...
            return tx_type_init.transmit_power_losses_lz_from_rule(mod, tx, tmp)
    ...
    def transmit_power_losses_lz_to_rule(mod, tx, tmp):
        ...
        if hasattr (imported_tx_operational_modules[tx_op_type],
            ...
            return tx_type_init.transmit_power_losses_lz_to_rule(mod, tx, tmp)
...
#################################################################################
    def transmit_H2_rule(mod, tx, tmp):
        ...
        if hasattr (imported_tx_operational_modules[tx_op_type],
                    'transmit_H2_rule'):
            return imported_tx_operational_modules[tx_op_type].\
                transmit_H2_rule(mod, tx, tmp)
        else:
            return tx_type_init.transmit_H2_rule(mod, tx, tmp)

    m.Transmit_H2_MW = Expression(
        m.TX_OPR_TMPS,
        rule=transmit_H2_rule
    )

    def transmit_H2_losses_lz_from_rule(mod, tx, tmp):
        tx_op_type = mod.tx_operational_type[tx]
        if hasattr (imported_tx_operational_modules[tx_op_type],
                    'transmit_H2_losses_lz_from_rule'):
            return imported_tx_operational_modules[tx_op_type].\
                transmit_H2_losses_lz_from_rule(mod, tx, tmp)
        else:
            return tx_type_init.transmit_H2_losses_lz_from_rule(mod, tx, tmp)
        
    m.Tx_Losses_LZ_H2_From_MW = Expression(
        m.TX_OPR_TMPS,
        rule=transmit_H2_losses_lz_from_rule
    )

    def transmit_H2_losses_lz_to_rule(mod, tx, tmp):
        tx_op_type = mod.tx_operational_type[tx]
        if hasattr (imported_tx_operational_modules[tx_op_type],
                    'transmit_H2_losses_lz_to_rule'):
            return imported_tx_operational_modules[tx_op_type].\
                transmit_H2_losses_lz_to_rule(mod, tx, tmp)
        else:
            return tx_type_init.transmit_H2_losses_lz_to_rule(mod, tx, tmp)
        
    m.Tx_Losses_LZ_H2_To_MW = Expression(
        m.TX_OPR_TMPS,
        rule=transmit_H2_losses_lz_to_rule
    )
...
#################################################################################
    def transmit_ccs_rule(mod, tx, tmp):
        ...
        if hasattr (imported_tx_operational_modules[tx_op_type],
                    'transmit_ccs_rule'):
            return imported_tx_operational_modules[tx_op_type].\
                transmit_ccs_rule(mod, tx, tmp)
        else:
            return tx_type_init.transmit_ccs_rule(mod, tx, tmp)

    m.Transmit_CCS_Tonne = Expression(
        m.TX_OPR_TMPS,
        rule=transmit_ccs_rule
    )
    '''
    def transmit_ccs_losses_lz_to_rule(mod, tx, tmp):
        tx_op_type = mod.tx_operational_type[tx]
        if hasattr (imported_tx_operational_modules[tx_op_type],
                    'transmit_ccs_losses_lz_to_rule'):
            return imported_tx_operational_modules[tx_op_type].\
                transmit_ccs_losses_lz_to_rule(mod, tx, tmp)
        else:
            return tx_type_init.transmit_ccs_losses_lz_to_rule(mod, tx, tmp)
        
    m.Tx_Losses_LZ_CCS_To_Tonne = Expression(
        m.TX_OPR_TMPS,
        rule=transmit_ccs_losses_lz_to_rule
    )

    def transmit_ccs_losses_lz_from_rule(mod, tx, tmp):
        tx_op_type = mod.tx_operational_type[tx]
        if hasattr (imported_tx_operational_modules[tx_op_type],
                    'transmit_ccs_losses_lz_from_rule'):
            return imported_tx_operational_modules[tx_op_type].\
                transmit_ccs_losses_lz_from_rule(mod, tx, tmp)
        else:
            return tx_type_init.transmit_ccs_losses_lz_from_rule(mod, tx, tmp)
        
    m.Tx_Losses_LZ_CCS_From_Tonne = Expression(
        m.TX_OPR_TMPS,
        rule=transmit_ccs_losses_lz_from_rule
    )
    '''
    def transmit_ccs_losses_lz_rule(mod, tx, tmp):
        tx_op_type = mod.tx_operational_type[tx]
        if hasattr (imported_tx_operational_modules[tx_op_type],
                    'transmit_ccs_losses_lz_rule'):
            return imported_tx_operational_modules[tx_op_type].\
                transmit_ccs_losses_lz_rule(mod, tx, tmp)
        else:
            return tx_type_init.transmit_ccs_losses_lz_rule(mod, tx, tmp)
        
    m.Tx_Losses_LZ_CCS_Tonne = Expression(
        m.TX_OPR_TMPS,
        rule=transmit_ccs_losses_lz_rule
    )
# Input-Output
###############################################################################
    ...
    with open(os.path.join(scenario_directory, str(subproblem), str(stage), "results",
        ...
        for (l, tmp) in m.TX_OPR_TMPS:
            writer.writerow([
                ...
                value(m.Transmit_H2_MW[l, tmp]),
                value(m.Tx_Losses_LZ_H2_From_MW[l, tmp]),
                value(m.Tx_Losses_LZ_H2_To_MW[l, tmp]),
                value(m.Transmit_CCS_Tonne[l, tmp]),
                value(m.Tx_Losses_LZ_CCS_Tonne[l, tmp])
            ])
