...
def export_results(scenario_directory, subproblem, stage, m, d):
    ...
    with open(os.path.join(scenario_directory, str(subproblem), str(stage), "results",
                           "ccsline_operations.csv"), "w", newline="") as \
            tx_op_results_file:
        ...
        for (l, tmp) in m.TX_OPR_TMPS:
            if m.tx_operational_type[l] == "ccs_simple":
                writer.writerow([
                    l,
                    m.load_zone_from[l],
                    m.load_zone_to[l],
                    ...
                    value(m.Tx_Losses_LZ_CCS_From_Tonne[l, tmp]),
                    value(m.Tx_Losses_LZ_CCS_To_Tonne[l, tmp])
                ])
        else:
            pass
...
def import_results_into_database(
    ...
    setup_results_import(
        conn=db, cursor=c,
        table="results_pipeline_operations",
        scenario_id=scenario_id, subproblem=subproblem, stage=stage
    )

    # Load results into the temporary table
    results = []
    with open(os.path.join(results_directory, "pipeline_operations.csv"),
              "r") as tx_op_file:
    ...
    insert_temp_sql = """
        INSERT INTO temp_results_pipeline_operations{}
        (scenario_id, transmission_line, period, subproblem_id, 
        stage_id, timepoint, timepoint_weight, 
        number_of_hours_in_timepoint,
        load_zone_from, load_zone_to, pipeline_flow_mw,
        pipeline_losses_lz_from, pipeline_losses_lz_to)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?);
        """.format(scenario_id)
    ...
    insert_sql = """
        INSERT INTO results_pipeline_operations
        ...
        load_zone_from, load_zone_to, pipeline_flow_mw,
        pipeline_losses_lz_from, pipeline_losses_lz_to
        FROM temp_results_pipeline_operations{}
...
def process_results(db, c, scenario_id, subscenarios, quiet):
    ...
    """
    if not quiet:
        print("aggregate pipeline imports exports")

    # Delete old results
    del_sql = """
        DELETE FROM results_pipeline_imports_exports_agg
        WHERE scenario_id = ?
        """
    ...
    agg_sql = """
        INSERT INTO results_pipeline_imports_exports_agg
        ...
        FROM (
            ...
            FROM (
                ...
                FROM results_pipeline_operations
        ...
        load_zone_to AS load_zone, spinup_or_lookahead,
        SUM(pipeline_flow_mw * timepoint_weight * 
        number_of_hours_in_timepoint) AS imports_pos_dir
        FROM results_pipeline_operations
        WHERE pipeline_flow_mw > 0
        AND scenario_id = ?
