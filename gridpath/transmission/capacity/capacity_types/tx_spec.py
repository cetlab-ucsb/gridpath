...
def add_model_components(
    ...
    m.tx_spec_min_flow_tonne = Param(
        m.TX_SPEC_OPR_PRDS,
        within=Reals
    )
    m.tx_spec_max_flow_tonne = Param(
        m.TX_SPEC_OPR_PRDS,
        within=Reals
    )
...
def min_transmission_capacity_tonne_rule(mod, tx, p):
    return mod.tx_spec_min_flow_tonne[tx, p]


def max_transmission_capacity_tonne_rule(mod, tx, p):
    return mod.tx_spec_max_flow_tonne[tx, p]
...
def load_model_data(m, d, data_portal, scenario_directory,
    ...
    data_portal.load(
        ...
        select=("transmission_line", "period",
                ...
                "specified_tx_min_tonne","specified_tx_max_tonne"),
        index=m.TX_SPEC_OPR_PRDS,
        param=(m.tx_spec_min_flow_mw,
               m.tx_spec_max_flow_mw,
               m.tx_spec_min_flow_tonne,
               m.tx_spec_max_flow_tonne)
    )
...
def get_model_inputs_from_database(
    ...
    tx_capacities = c.execute(
        """SELECT transmission_line, period, min_mw, max_mw, min_tonne, max_tonne
    ...
    with open(os.path.join(scenario_directory, str(subproblem), str(stage), "inputs",
        ...
        writer.writerow(
            ["transmission_line", "period",
             "specified_tx_min_mw",
             "specified_tx_max_mw",
             "specified_tx_min_tonne",
             "specified_tx_max_tonne"]
        )
