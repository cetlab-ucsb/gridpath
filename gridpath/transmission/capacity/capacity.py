...
def add_model_components(m, d, scenario_directory, subproblem, stage):
    ...
    def tx_min_capacity_tonne_rule(mod, tx, p):
        tx_cap_type = mod.tx_capacity_type[tx]
        return imported_tx_capacity_modules[tx_cap_type]. \
            min_transmission_capacity_tonne_rule(mod, tx, p)

    def tx_max_capacity_tonne_rule(mod, tx, p):
        tx_cap_type = mod.tx_capacity_type[tx]
        return imported_tx_capacity_modules[tx_cap_type]. \
            max_transmission_capacity_tonne_rule(mod, tx, p)
    ...
    m.Tx_Min_Capacity_Tonne = Expression(
        m.TX_OPR_PRDS,
        rule=tx_min_capacity_tonne_rule
    )

    m.Tx_Max_Capacity_Tonne = Expression(
        m.TX_OPR_PRDS,
        rule=tx_max_capacity_tonne_rule
    )
...
def export_results(scenario_directory, subproblem, stage, m, d):
    ...
    with open(os.path.join(scenario_directory, str(subproblem), str(stage), "results",
        ...
        writer.writerow(["tx_line", "period", "load_zone_from", "load_zone_to",
                         ...
                         "transmission_min_capacity_tonne",
                         "transmission_max_capacity_tonne"])
        for (tx_line, p) in m.TX_OPR_PRDS:
            writer.writerow([
                ...
                value(m.Tx_Min_Capacity_Tonne[tx_line, p]),
                value(m.Tx_Max_Capacity_Tonne[tx_line, p])
            ])
