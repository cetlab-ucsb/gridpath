...
def add_model_components(m, d, scenario_directory, subproblem, stage):
    ...
    m.H2_mw = Param(m.LOAD_ZONES, m.TMPS,
                             within=NonNegativeReals)
...
def load_model_data(m, d, data_portal, scenario_directory, subproblem, stage):
    ...
    data_portal.load(
        ...
        param=(m.static_load_mw,m.H2_mw)
    )
...
def get_inputs_from_database(scenario_id, subscenarios, subproblem, stage, conn):
    ...
    loads = c.execute(
        """SELECT load_zone, timepoint, load_mw, H2_mw
...
def write_model_inputs(scenario_directory, scenario_id, subscenarios, subproblem, stage, conn):
    ...
    with open(os.path.join(scenario_directory, str(subproblem), str(stage), "inputs",
        ...
        # Write header
        writer.writerow(
            ["LOAD_ZONES", "timepoint", "load_mw","H2_mw"]
        )
    ...
    '''
    with open(os.path.join(scenario_directory, str(subproblem), str(stage), "inputs",
                           "H2_mw.tab"), "w", newline="") as \
            H2_tab_file:
        writer = csv.writer(H2_tab_file, delimiter="\t", lineterminator="\n")

        # Write header
        writer.writerow(
            ["LOAD_ZONES", "timepoint", "H2_mw"]
        )
