...
def add_model_components(m, d, scenario_directory, subproblem, stage):
    ...
    #add hydrogen module
    def total_ccs_production_rule(mod, z, tmp):
        return sum(mod.CCS_Capture_Tonne[g, tmp]
                   for g in mod.OPR_PRJS_IN_TMP[tmp]
                   if mod.load_zone[g] == z)
    
    m.CCS_Production_in_Zone_Tonne = \
        Expression(m.LOAD_ZONES, m.TMPS,
                   rule=total_ccs_production_rule)

    def total_ccs_consumption_rule(mod, z, tmp):
        return sum(mod.CCS_Storage_Tonne[g, tmp]
                   for g in mod.OPR_PRJS_IN_TMP[tmp]
                   if mod.load_zone[g] == z)
    
    m.CCS_Consumption_in_Zone_Tonne = \
        Expression(m.LOAD_ZONES, m.TMPS,
                   rule=total_ccs_consumption_rule)
