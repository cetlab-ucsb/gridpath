        ...
        **Defined Over**: PRJ_OPR_TMPS
        ...
        if hasattr(imported_operational_modules[gen_op_type],
            ...
            return imported_operational_modules[gen_op_type].\
                H2_provision_rule(mod, prj, tmp)
        else:
            return op_type_init.H2_provision_rule(mod, prj, tmp)
     
    m.H2_Provision_MW = Expression(
        m.PRJ_OPR_TMPS,
        rule=H2_provision_rule
    )
...
def export_results(scenario_directory, subproblem, stage, m, d):
    ...
    with open(os.path.join(scenario_directory, str(subproblem), str(stage), "results",
                           "dispatch_H2.csv"), "w", newline="") as f:
        writer = csv.writer(f)
        writer.writerow(["project", "period", "horizon", "timepoint",
                         "operational_type", "balancing_type",
                         "timepoint_weight", "number_of_hours_in_timepoint",
                         "load_zone", "technology", "H2_mw"])
        for (p, tmp) in m.PRJ_OPR_TMPS:
            if m.operational_type[p] in ["gen_H2","stor_H2","gen_H2_ele","gen_commit_cap_H2","gen_commit_cap_H2_ccs"]:
                writer.writerow([
                    p,
                    m.period[tmp],
...
def summarize_results(scenario_directory, subproblem, stage):
    ...
    operational_results_df = pd.read_csv(
        os.path.join(scenario_directory, str(subproblem), str(stage),
                     "results", "dispatch_H2.csv")
    )
    try:
        operational_results_df["weighted_H2_mwh"] = \
            operational_results_df["H2_mw"] \
        ...
        operational_results_agg_df.columns = ["weighted_H2_mwh"]
        ...
        for indx, row in operational_results_agg_df.iterrows():
            if lz_period_power_df.weighted_H2_mwh[indx[0], indx[1]] == 0:
                pct = 0
            else:
                pct = operational_results_agg_df.weighted_H2_mwh[indx] \
                    / lz_period_power_df.weighted_H2_mwh[indx[0], indx[1]] \
                    * 100.0
