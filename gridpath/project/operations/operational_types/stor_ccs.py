...
def add_model_components(m, d, scenario_directory, subproblem, stage):
    ...
    m.STOR_STOR_CCS = Set(
        ...
        initialize=lambda mod: subset_init_by_param_value(
            mod, "PROJECTS", "operational_type", "stor_ccs"
        )
    )

    m.STOR_STOR_CCS_OPR_TMPS = Set(
        dimen=2, within=m.PRJ_OPR_TMPS,
        initialize=lambda mod: list(
            set((g, tmp) for (g, tmp) in mod.PRJ_OPR_TMPS
                if g in mod.STOR_STOR_CCS)
        )
    )

    m.STOR_STOR_CCS_LINKED_TMPS = Set(dimen=2)

    # Required Params
    ###########################################################################
    m.stor_ccs_variable_ccs_om_cost_per_tonne = Param(
        m.STOR_STOR_CCS,
        within=NonNegativeReals
        )

    #Optional Parmas
    m.stor_ccs_ccs_mwh_per_tonne = Param(
        m.STOR_STOR_CCS,
        within=NonNegativeReals,
        default=0
        )
    ...
    m.stor_linked_starting_ccs_in_storage = Param(
        m.STOR_STOR_CCS_LINKED_TMPS,
        within=NonNegativeReals
    )

    m.stor_ccs_linked_charge = Param(
        m.STOR_STOR_CCS_LINKED_TMPS,
        within=NonNegativeReals
    )
    ...
    m.Stor_CCS_Charge_Tonne = Var(
        m.STOR_STOR_CCS_OPR_TMPS,
        within=NonNegativeReals
    ) 
    ...
    m.Stor_Max_CCS_in_Storage_Constraint = Constraint(
        m.STOR_STOR_CCS_OPR_TMPS,
        rule=max_ccs_in_storage_rule
    )
...
def max_ccs_in_storage_rule(mod, s, tmp):
    """
    ...
    **Enforced Over**: STOR_OPR_TMPS
    ...
    """
    return sum(mod.Stor_CCS_Charge_Tonne[s,v] * mod.hrs_in_tmp[v] * mod.tmp_weight[v] for (gen,v) in mod.STOR_STOR_CCS_OPR_TMPS if gen==s)\
        <= mod.CCS_Mass_Tonne[s, mod.period[tmp]] \
...
def power_provision_rule(mod, s, tmp):
    ...
    return -mod.Stor_CCS_Charge_Tonne[s,tmp] * mod.stor_ccs_ccs_mwh_per_tonne[s]

def ccs_storage_rule(mod, s, tmp):
    """
    ...
    storage is empty).
    """
    return mod.Stor_CCS_Charge_Tonne[s, tmp]
...
def variable_om_cost_rule(mod, g, tmp):
    ...
    """
    return mod.Stor_CCS_Charge_Tonne[g, tmp] * mod.stor_ccs_variable_ccs_om_cost_per_tonne[g]
...
def export_results(mod, d, scenario_directory, subproblem, stage):
    ...
    with open(os.path.join(scenario_directory, str(subproblem), str(stage), "results",
                           "dispatch_stor_ccs.csv"), "w", newline="") as f:
        writer = csv.writer(f)
        writer.writerow(["project", "period", "balancing_type_project",
                         "horizon", "timepoint", "timepoint_weight",
                         "number_of_hours_in_timepoint",
                         "technology", "load_zone",
                         "ccs_storage_charge_tonne"])
        for (p, tmp) in mod.STOR_STOR_CCS_OPR_TMPS:
            writer.writerow([
                p,
                mod.period[tmp],
                ...
                mod.technology[p],
                mod.load_zone[p],
                value(mod.Stor_CCS_Charge_Tonne[p, tmp])
            ])
...
def validate_inputs(scenario_id, subscenarios, subproblem, stage, conn):
    ...
    validate_opchars(scenario_id, subscenarios, subproblem, stage, conn, "stor_ccs")
