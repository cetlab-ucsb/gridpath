...
def add_model_components(m, d, scenario_directory, subproblem, stage):
    ...
    # Sets
    ###########################################################################
    m.GEN_COMMIT_CAP_H2_CCS = Set(
        within=m.PROJECTS,
        initialize=lambda mod: subset_init_by_param_value(
            mod, "PROJECTS", "operational_type", "gen_commit_cap_H2_ccs"
        )
    )

    m.GEN_COMMIT_CAP_H2_CCS_OPR_TMPS = Set(
        dimen=2,
        within=m.PRJ_OPR_TMPS,
        initialize=lambda mod: list(
            set((g, tmp) for (g, tmp) in mod.PRJ_OPR_TMPS
                if g in mod.GEN_COMMIT_CAP_H2_CCS)
        )
    )

    # Required Params
    ###########################################################################
    m.gen_commit_cap_H2_ccs_unit_size_mw = Param(
        m.GEN_COMMIT_CAP_H2_CCS,
        within=NonNegativeReals
    )
    m.gen_commit_cap_H2_ccs_min_stable_level_fraction = Param(
        m.GEN_COMMIT_CAP_H2_CCS,
        within=PercentFraction
    )

    m.gen_commit_cap_H2_ccs_ccs_efficiency = Param(
        m.GEN_COMMIT_CAP_H2_CCS,
        within=PercentFraction
        )

    m.gen_commit_cap_H2_ccs_ccs_mwh_per_tonne = Param(
        m.GEN_COMMIT_CAP_H2_CCS,
        within=NonNegativeReals
        )
    
    m.gen_commit_cap_H2_ccs_variable_ccs_om_cost_per_tonne = Param(
        m.GEN_COMMIT_CAP_H2_CCS,
        within=NonNegativeReals
        )
    ...
    # Variables
    ###########################################################################
    m.GenCommitCap_CCS_Provide_H2_MW = Var(
        m.GEN_COMMIT_CAP_H2_CCS_OPR_TMPS,
        within=NonNegativeReals
    )
    m.Commit_Capacity_CCS_H2_MW = Var(
        m.GEN_COMMIT_CAP_H2_CCS_OPR_TMPS,
        within=NonNegativeReals
    )
    #CCS operation capacity
    m.Gen_Capacity_CCS_H2_Tonne = Var(
        m.GEN_COMMIT_CAP_H2_CCS_OPR_TMPS,
        within=NonNegativeReals
    )
    ...
    # Commitment and power
    m.Commit_Capacity_CCS_H2_Constraint = Constraint(
        m.GEN_COMMIT_CAP_H2_CCS_OPR_TMPS,
        rule=commit_capacity_H2_constraint_rule
    )

    m.GenCommitCap_CCS_Max_H2_Constraint = Constraint(
        m.GEN_COMMIT_CAP_H2_CCS_OPR_TMPS,
        rule=max_H2_rule
    )

    m.GenCommitCap_CCS_Min_H2_Constraint = Constraint(
        m.GEN_COMMIT_CAP_H2_CCS_OPR_TMPS,
        rule=min_H2_rule
    )
    ...
    '''
    m.Gen_Capacity_CCS_H2_Tonne_Constraint = Constraint(
        m.GEN_COMMIT_CAP_H2_CCS_OPR_TMPS,
        rule=capacity_ccs_H2_max_constraint_rule
    )
...
def commit_capacity_H2_constraint_rule(mod, g, tmp):
    ...
    **Enforced Over**: GEN_COMMIT_CAP_OPR_TMPS
    ...
    """
    return mod.Commit_Capacity_CCS_H2_MW[g, tmp] \
...
def max_H2_rule(mod, g, tmp):
    ...
    **Enforced Over**: GEN_COMMIT_CAP_OPR_TMPS
    ...
    """
    return mod.GenCommitCap_CCS_Provide_H2_MW[g, tmp] \
        <= mod.Commit_Capacity_CCS_H2_MW[g, tmp]
...
def min_H2_rule(mod, g, tmp):
    ...
    **Enforced Over**: GEN_COMMIT_CAP_OPR_TMPS
    ...
    return mod.GenCommitCap_CCS_Provide_H2_MW[g, tmp] \
        >= mod.Commit_Capacity_CCS_H2_MW[g, tmp] \
        * mod.gen_commit_cap_H2_ccs_min_stable_level_fraction[g]


    

def capacity_ccs_H2_max_constraint_rule(mod, g, tmp):
    """
    **Constraint Name**: Commit_Capacity_Constraint
    **Enforced Over**: GEN_COMMIT_CAP_OPR_TMPS
    ...
    Can't commit more capacity than available in each timepoint.
    """
    return mod.Gen_Capacity_CCS_H2_Tonne[g, tmp] \
...
def power_provision_rule(mod, g, tmp):
    ...
    return - mod.Gen_Capacity_CCS_H2_Tonne[g,tmp]*mod.gen_commit_cap_H2_ccs_ccs_mwh_per_tonne[g]  
...
def fuel_burn_by_ll_rule(mod, g, tmp, s):
    ...
    return \
        ...
        * mod.GenCommitCap_CCS_Provide_H2_MW[g, tmp] \
...
def ccs_efficiency_rule(mod, g):
    return mod.gen_commit_cap_H2_ccs_ccs_efficiency[g]
...
def variable_om_cost_rule(mod, g, tmp):
    ...
    return mod.GenCommitCap_CCS_Provide_H2_MW[g, tmp] \
        * mod.variable_om_cost_per_mwh[g] \
        + mod.Gen_Capacity_CCS_H2_Tonne[g,tmp] \
        * mod.gen_commit_cap_H2_ccs_variable_ccs_om_cost_per_tonne[g]
...
def load_model_data(
    ...
    load_optype_model_data(
        ...
        stage=stage, op_type="gen_commit_cap_H2_ccs"
    )
    '''
...
def export_results(
    ...
    with open(os.path.join(scenario_directory, str(subproblem), str(stage), "results",
                           "dispatch_capacity_commit_H2_ccs.csv"),
              "w", newline="") as f:
        writer = csv.writer(f)
        ...
        for (p, tmp) \
                in mod. \
                GEN_COMMIT_CAP_H2_CCS_OPR_TMPS:
            writer.writerow([
                p,
                mod.period[tmp],
                ...
                value(mod.Gen_Capacity_CCS_H2_Tonne[p,tmp]) * value (mod.gen_commit_cap_H2_ccs_ccs_mwh_per_tonne[p]),
                value(mod.Commit_Capacity_CCS_H2_MW[p, tmp]),
                value(mod.Gen_Capacity_CCS_H2_Tonne[p,tmp]),
                value(mod.Commit_Capacity_CCS_H2_MW[p, tmp]) /
                mod.gen_commit_cap_H2_ccs_unit_size_mw[p]
            ])
...
def import_model_results_to_database(
    ...
    update_dispatch_results_table(
        ...
        results_file="dispatch_capacity_commit_H2_ccs.csv"
    )
...
def validate_inputs(scenario_id, subscenarios, subproblem, stage, conn):
    ...
    validate_opchars(scenario_id, subscenarios, subproblem, stage, conn, "gen_commit_cap_H2_ccs")
