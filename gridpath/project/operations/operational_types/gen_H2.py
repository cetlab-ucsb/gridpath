...
def add_model_components(m, d, scenario_directory, subproblem, stage):
    ...
    m.GEN_H2 = Set(
        ...
        initialize=lambda mod: subset_init_by_param_value(
            mod, "PROJECTS", "operational_type", "gen_H2"
        )
    )

    m.GEN_H2_OPR_TMPS = Set(
    ...
    m.GEN_H2_LINKED_TMPS = Set(dimen=2)

    # Required Params
    ###########################################################################

    m.gen_H2_charging_efficiency = Param(
        m.GEN_H2, within=PercentFraction
    )

    # Optional Params
    ###########################################################################

    m.gen_H2_charging_capacity_multiplier = Param(
        m.GEN_H2, within=NonNegativeReals, default=1.0
    )
    ...
    m.Gen_H2_Max_Charge_Constraint = Constraint(
        m.GEN_H2_OPR_TMPS,
        rule=max_charge_rule
    )

    m.Gen_H2_MW_Constraint = Constraint(
        m.GEN_H2_OPR_TMPS,
        rule=H2_generation_rule
    )
...
def max_charge_rule(mod, s, tmp):
    ...
    **Enforced Over**: STOR_OPR_TMPS
    ...
    """
    return mod.Gen_H2_Charge_MW[s, tmp] \
        <= mod.Capacity_MW[s, mod.period[tmp]] \
        * mod.Availability_Derate[s, tmp] \
        * mod.gen_H2_charging_capacity_multiplier[s]

def H2_generation_rule(mod, s, tmp):

        return \
            mod.Gen_H2_MW[s, tmp] \
            == mod.gen_H2_charging_efficiency[s]\
            *mod.Gen_H2_Charge_MW[s,tmp]
...
def variable_om_cost_rule(mod, g, tmp):
    ...
    """
    return mod.Gen_H2_MW[g, tmp] * mod.variable_om_cost_per_mwh[g]
...
def export_results(mod, d,
    ...
    with open(os.path.join(scenario_directory, str(subproblem), str(stage), "results",
                           "dispatch_gen_H2.csv"), "w", newline="") as f:
        writer = csv.writer(f)
        writer.writerow(["project", "period", "balancing_type_project",
                         ...
                         "charge_mw", "H2_mw"])
        for (p, tmp) in mod.GEN_H2_OPR_TMPS:
            writer.writerow([
                p,
                mod.period[tmp],
                ...
                mod.technology[p],
                mod.load_zone[p],
                value(mod.Gen_H2_Charge_MW[p, tmp]),
                value(mod.Gen_H2_MW[p, tmp])
            ])
...
def validate_inputs(scenario_id, subscenarios, subproblem, stage, conn):
    ...
    validate_opchars(scenario_id, subscenarios, subproblem, stage, conn, "gen_H2")
