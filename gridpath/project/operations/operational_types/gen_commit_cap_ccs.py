...
def add_model_components(m, d, scenario_directory, subproblem, stage):
    ...
    # Sets
    ###########################################################################
    m.GEN_COMMIT_CAP_CCS = Set(
        within=m.PROJECTS,
        initialize=lambda mod: subset_init_by_param_value(
            mod, "PROJECTS", "operational_type", "gen_commit_cap_ccs"
        )
    )

    m.GEN_COMMIT_CAP_CCS_OPR_TMPS = Set(
        dimen=2,
        within=m.PRJ_OPR_TMPS,
        initialize=lambda mod: list(
            set((g, tmp) for (g, tmp) in mod.PRJ_OPR_TMPS
                if g in mod.GEN_COMMIT_CAP_CCS)
        )
    )

    m.GEN_COMMIT_CAP_CCS_LINKED_TMPS = Set(dimen=2)

    # Required Params
    ###########################################################################
    m.gen_commit_cap_ccs_unit_size_mw = Param(
        m.GEN_COMMIT_CAP_CCS,
        within=NonNegativeReals
    )
    m.gen_commit_cap_ccs_min_stable_level_fraction = Param(
        m.GEN_COMMIT_CAP_CCS,
    ...
    m.gen_commit_cap_ccs_variable_ccs_om_cost_per_tonne = Param(
        m.GEN_COMMIT_CAP_CCS,
        within=NonNegativeReals
        )
    ...
    m.gen_commit_cap_ccs_startup_plus_ramp_up_rate = Param(
        m.GEN_COMMIT_CAP_CCS,
        within=PercentFraction,
        default=1
    )
    m.gen_commit_cap_ccs_shutdown_plus_ramp_down_rate = Param(
        m.GEN_COMMIT_CAP_CCS,
        within=PercentFraction,
        default=1
    )
    m.gen_commit_cap_ccs_ramp_up_when_on_rate = Param(
        m.GEN_COMMIT_CAP_CCS,
        within=PercentFraction,
        default=1
    )
    m.gen_commit_cap_ccs_ramp_down_when_on_rate = Param(
        m.GEN_COMMIT_CAP_CCS,
        within=PercentFraction,
        default=1
    )
    m.gen_commit_cap_ccs_min_up_time_hours = Param(
        m.GEN_COMMIT_CAP_CCS,
        within=NonNegativeReals,
        default=1
    )
    m.gen_commit_cap_ccs_min_down_time_hours = Param(
        m.GEN_COMMIT_CAP_CCS,
        within=NonNegativeReals,
        default=1
    )

    m.gen_commit_cap_ccs_aux_consumption_frac_capacity = Param(
        m.GEN_COMMIT_CAP_CCS,
        within=PercentFraction,
        default=0
    )

    m.gen_commit_cap_ccs_aux_consumption_frac_power = Param(
        m.GEN_COMMIT_CAP_CCS,
        within=PercentFraction,
        default=0
    )
    ...
    m.gen_commit_cap_ccs_linked_commit_capacity = Param(
        m.GEN_COMMIT_CAP_CCS_LINKED_TMPS,
        within=NonNegativeReals
    )

    m.gen_commit_cap_ccs_linked_power = Param(
        m.GEN_COMMIT_CAP_CCS_LINKED_TMPS,
        within=NonNegativeReals
    )

    m.gen_commit_cap_ccs_linked_upwards_reserves = Param(
        m.GEN_COMMIT_CAP_CCS_LINKED_TMPS,
        within=NonNegativeReals
    )

    m.gen_commit_cap_ccs_linked_downwards_reserves = Param(
        m.GEN_COMMIT_CAP_CCS_LINKED_TMPS,
        within=NonNegativeReals
    )

    m.gen_commit_cap_ccs_linked_startup = Param(
        m.GEN_COMMIT_CAP_CCS_LINKED_TMPS,
        within=NonNegativeReals
    )

    m.gen_commit_cap_ccs_linked_shutdown = Param(
        m.GEN_COMMIT_CAP_CCS_LINKED_TMPS,
        within=NonNegativeReals
    )
    ...
    # Variables
    ###########################################################################
    m.GenCommitCap_CCS_Provide_Power_MW = Var(
        m.GEN_COMMIT_CAP_CCS_OPR_TMPS,
        within=NonNegativeReals
    )
    m.Commit_Capacity_CCS_MW = Var(
        m.GEN_COMMIT_CAP_CCS_OPR_TMPS,
        within=NonNegativeReals
    )
    #CCS operation capacity
    m.Gen_Capacity_CCS_Tonne = Var(
        m.GEN_COMMIT_CAP_CCS_OPR_TMPS,
        within=NonNegativeReals
    )
    ...
    m.Ramp_Up_Startup_CCS_MW = Var(
        m.GEN_COMMIT_CAP_CCS_OPR_TMPS,
        within=Reals
    )
    m.Ramp_Down_Shutdown_CCS_MW = Var(
        m.GEN_COMMIT_CAP_CCS_OPR_TMPS,
        within=Reals
    )

    m.Ramp_Up_When_On_CCS_MW = Var(
        m.GEN_COMMIT_CAP_CCS_OPR_TMPS,
        within=NonNegativeReals
    )
    m.Ramp_Down_When_On_CCS_MW = Var(
        m.GEN_COMMIT_CAP_CCS_OPR_TMPS,
    ...
    m.GenCommitCap_CCS_Startup_MW = Var(
        m.GEN_COMMIT_CAP_CCS_OPR_TMPS,
        within=NonNegativeReals
    )
    m.GenCommitCap_CCS_Shutdown_MW = Var(
        m.GEN_COMMIT_CAP_CCS_OPR_TMPS,
        within=NonNegativeReals
    )
    ...
    m.GenCommitCap_CCS_Upwards_Reserves_MW = Expression(
        m.GEN_COMMIT_CAP_CCS_OPR_TMPS,
    ...
    m.GenCommitCap_CCS_Downwards_Reserves_MW = Expression(
        m.GEN_COMMIT_CAP_CCS_OPR_TMPS,
        rule=downwards_reserve_rule
    )

    m.GenCommitCap_CCS_Auxiliary_Consumption_MW = Expression(
        m.GEN_COMMIT_CAP_CCS_OPR_TMPS,
    ...
    m.GenCommitCap_CCS_Min_Power_Constraint = Constraint(
        m.GEN_COMMIT_CAP_CCS_OPR_TMPS,
        rule=min_power_rule
    )
    ...
    m.GenCommitCap_CCS_Constraint = Constraint(
        m.GEN_COMMIT_CAP_CCS_OPR_TMPS_FUEL,
    ...
    m.Ramp_Up_When_On_CCS_Constraint = Constraint(
        m.GEN_COMMIT_CAP_CCS_OPR_TMPS,
        rule=ramp_up_on_to_on_constraint_rule
    )

    m.Ramp_Up_When_On_Headroom_CCS_Constraint = Constraint(
        m.GEN_COMMIT_CAP_CCS_OPR_TMPS,
        rule=ramp_up_on_to_on_headroom_constraint_rule
    )

    m.GenCommitCap_Ramp_Up_CCS_Constraint = Constraint(
        m.GEN_COMMIT_CAP_CCS_OPR_TMPS,
        rule=ramp_up_constraint_rule
    )

    m.Ramp_Down_On_to_Off_CCS_Constraint = Constraint(
        m.GEN_COMMIT_CAP_CCS_OPR_TMPS,
        rule=ramp_down_on_to_off_constraint_rule
    )

    m.Ramp_Down_When_On_CCS_Constraint = Constraint(
        m.GEN_COMMIT_CAP_CCS_OPR_TMPS,
        rule=ramp_down_on_to_on_constraint_rule
    )

    m.Ramp_Down_When_On_Headroom_CCS_Constraint = Constraint(
        m.GEN_COMMIT_CAP_CCS_OPR_TMPS,
        rule=ramp_down_on_to_on_headroom_constraint_rule
    )

    m.GenCommitCap_CCS_Ramp_Down_CCS_Constraint = Constraint(
        m.GEN_COMMIT_CAP_CCS_OPR_TMPS,
        rule=ramp_down_constraint_rule
    )

    # Min up and down time
    m.GenCommitCap_CCS_Startup_Constraint = Constraint(
        m.GEN_COMMIT_CAP_CCS_OPR_TMPS,
        rule=startup_constraint_rule
    )

    m.GenCommitCap_CCS_Shutdown_Constraint = Constraint(
        m.GEN_COMMIT_CAP_CCS_OPR_TMPS,
        rule=shutdown_constraint_rule
    )

    m.GenCommitCap_CCS_Min_Up_Time_Constraint = Constraint(
        m.GEN_COMMIT_CAP_CCS_OPR_TMPS,
        rule=min_up_time_constraint_rule
    )

    m.GenCommitCap_CCS_Min_Down_Time_Constraint = Constraint(
        m.GEN_COMMIT_CAP_CCS_OPR_TMPS,
...
def auxiliary_consumption_rule(mod, g, tmp):
    ...
    **Defined Over**: GEN_COMMIT_CAP_OPR_TMPS
    """
    return mod.Commit_Capacity_CCS_MW[g, tmp] \
        * mod.gen_commit_cap_ccs_aux_consumption_frac_capacity[g] \
        + mod.GenCommitCap_CCS_Provide_Power_MW[g, tmp] * \
        mod.gen_commit_cap_ccs_aux_consumption_frac_power[g]
...
def max_power_rule(mod, g, tmp):
    ...
    **Enforced Over**: GEN_COMMIT_CAP_OPR_TMPS
    ...
    return mod.GenCommitCap_CCS_Provide_Power_MW[g, tmp] \
        + mod.GenCommitCap_CCS_Upwards_Reserves_MW[g, tmp] \
        <= mod.Commit_Capacity_CCS_MW[g, tmp]
...
def min_power_rule(mod, g, tmp):
    ...
    **Enforced Over**: GEN_COMMIT_CAP_OPR_TMPS
    ...
    """
    return mod.GenCommitCap_CCS_Provide_Power_MW[g, tmp] \
        - mod.GenCommitCap_CCS_Downwards_Reserves_MW[g, tmp] \
        >= mod.Commit_Capacity_CCS_MW[g, tmp] \
...
def gen_commit_capacity_ccs_constraint_rule(mod, g, tmp,s):
    ...
    **Enforced Over**: GEN_COMMIT_CAP_OPR_TMPS
    ...
    Can't commit more capacity than available in each timepoint.
    """
    Operation_fuel = mod.fuel_burn_slope_mmbtu_per_mwh[g, mod.period[tmp], s] \
        * mod.GenCommitCap_CCS_Provide_Power_MW[g, tmp] \
        + mod.fuel_burn_intercept_mmbtu_per_mw_hr[g, mod.period[tmp], s] \
        * mod.Commit_Capacity_CCS_MW[g, tmp]

    Startup_fuel= mod.GenCommitCap_CCS_Startup_MW[g, tmp] * mod.startup_fuel_mmbtu_per_mw[g]

    Total_fuel=Operation_fuel + Startup_fuel
    
    return mod.ccs_efficiency[prj] * \
           Total_fuel * \
           mod.co2_intensity_tons_per_mmbtu[mod.fuel[prj]]\
           >= mod.Gen_Capacity_CCS_Tonne[prj,tmp]
'''

def capacity_ccs_max_constraint_rule(mod, g, tmp):
    """
    **Constraint Name**: Commit_Capacity_Constraint
    **Enforced Over**: GEN_COMMIT_CAP_OPR_TMPS
    ...
    if check_if_boundary_type_and_first_timepoint(
        ...
        boundary_type="linked"
            ...
            prev_tmp_commit_capacity = \
                mod.gen_commit_cap_ccs_linked_commit_capacity[g, 0]
        else:
            prev_tmp_hrs_in_tmp = mod.hrs_in_tmp[
                    mod.prev_tmp[tmp, mod.balancing_type_project[g]]
            ]
            prev_tmp_commit_capacity = \
                mod.Commit_Capacity_CCS_MW[
                    g, mod.prev_tmp[tmp, mod.balancing_type_project[g]]
                ]

        return mod.Ramp_Up_Startup_CCS_MW[g, tmp] \
            <= \
            (mod.Commit_Capacity_CCS_MW[g, tmp] - prev_tmp_commit_capacity) \
            * mod.gen_commit_cap_ccs_startup_plus_ramp_up_rate[g] * 60 \
            * prev_tmp_hrs_in_tmp
...
def ramp_up_on_to_on_constraint_rule(mod, g, tmp):
    ...
    **Enforced Over**: GEN_COMMIT_CAP_OPR_TMPS
    ...
    if check_if_boundary_type_and_first_timepoint(
        ...
        return mod.Ramp_Up_When_On_CCS_MW[g, tmp] \
            <= \
            prev_tmp_commit_capacity \
            * mod.gen_commit_cap_ccs_ramp_up_when_on_rate[g] * 60 \
            * prev_tmp_hrs_in_tmp
    ...
    if check_if_boundary_type_and_first_timepoint(
        ...
        boundary_type="linked"
            ...
            prev_tmp_commit_capacity = \
                mod.gen_commit_cap_ccs_linked_commit_capacity[g, 0]
            prev_tmp_power = \
                mod.gen_commit_cap_ccs_linked_power[g, 0]
            prev_tmp_downwards_reserves = \
                mod.gen_commit_cap_ccs_linked_downwards_reserves[g, 0]
        else:
            prev_tmp_commit_capacity = mod.Commit_Capacity_CCS_MW[
                   g, mod.prev_tmp[tmp, mod.balancing_type_project[g]]
            ]
            prev_tmp_power = mod.GenCommitCap_CCS_Provide_Power_MW[
                g, mod.prev_tmp[tmp, mod.balancing_type_project[g]]
            ]
            prev_tmp_downwards_reserves = \
                mod.GenCommitCap_CCS_Downwards_Reserves_MW[
                    g, mod.prev_tmp[tmp, mod.balancing_type_project[g]]
                ]
        return mod.Ramp_Up_When_On_CCS_MW[g, tmp] \
            <= \
    ...
    **Enforced Over**: GEN_COMMIT_CAP_OPR_TMPS
    ...
    if check_if_boundary_type_and_first_timepoint(
        ...
        boundary_type="linked"
            ...
            prev_tmp_power = \
                mod.GenCommitCap_CCS_Provide_Power_MW[
                    g, mod.prev_tmp[tmp, mod.balancing_type_project[g]]
                ]
            prev_tmp_downwards_reserves = \
                mod.GenCommitCap_CCS_Downwards_Reserves_MW[
        ...
        if (
                ...
                * prev_tmp_hrs_in_tmp
                >= 1
                and mod.gen_commit_cap_ccs_ramp_up_when_on_rate[g] * 60
                * prev_tmp_hrs_in_tmp
                >= (1 - mod.gen_commit_cap_ccs_min_stable_level_fraction[g])
        ):
            return Constraint.Skip
        else:
            return (mod.GenCommitCap_CCS_Provide_Power_MW[g, tmp]
                ...
                - (prev_tmp_power - prev_tmp_downwards_reserves) \
                <= \
                mod.Ramp_Up_Startup_CCS_MW[g, tmp] \
                + mod.Ramp_Up_When_On_CCS_MW[g, tmp]
...
def ramp_down_on_to_off_constraint_rule(mod, g, tmp):
    ...
    **Enforced Over**: GEN_COMMIT_CAP_OPR_TMPS
    ...
    if check_if_boundary_type_and_first_timepoint(
        ...
        return mod.Ramp_Down_Shutdown_CCS_MW[g, tmp] \
            >= \
            (mod.Commit_Capacity_CCS_MW[g, tmp] - prev_tmp_commit_capacity) \
            * mod.gen_commit_cap_ccs_shutdown_plus_ramp_down_rate[g] * 60 \
            * prev_tmp_hrs_in_tmp
...
def ramp_down_on_to_on_constraint_rule(mod, g, tmp):
    ...
    **Enforced Over**: GEN_COMMIT_CAP_OPR_TMPS
    ...
    if check_if_boundary_type_and_first_timepoint(
        ...
        return mod.Ramp_Down_When_On_CCS_MW[g, tmp] \
            >= \
            mod.Commit_Capacity_CCS_MW[g, tmp] \
            * (-mod.gen_commit_cap_ccs_ramp_down_when_on_rate[g]) * 60 \
            * prev_tmp_hrs_in_tmp
    ...
    if check_if_boundary_type_and_first_timepoint(
        ...
        boundary_type="linked"
            ...
            prev_tmp_upwards_reserves = \
                mod.gen_commit_cap_ccs_linked_upwards_reserves[g, 0]
        else:
            prev_tmp_hrs_in_tmp = mod.hrs_in_tmp[
                  mod.prev_tmp[tmp, mod.balancing_type_project[g]]
            ]
            ...
            prev_tmp_upwards_reserves = mod.GenCommitCap_CCS_Upwards_Reserves_MW[
                    g, mod.prev_tmp[tmp, mod.balancing_type_project[g]]]
        ...
        if (
            ...
            return (mod.GenCommitCap_CCS_Provide_Power_MW[g, tmp]
                ...
                >= \
                mod.Ramp_Down_Shutdown_CCS_MW[g, tmp] \
                + mod.Ramp_Down_When_On_CCS_MW[g, tmp]
...
def startup_constraint_rule(mod, g, tmp):
    ...
    **Enforced Over**: GEN_COMMIT_CAP_OPR_TMPS
    ...
    if check_if_boundary_type_and_first_timepoint(
        ...
        return mod.GenCommitCap_CCS_Startup_MW[g, tmp] \
            >= mod.Commit_Capacity_CCS_MW[g, tmp] - prev_tmp_commit_capacity
...
def shutdown_constraint_rule(mod, g, tmp):
    ...
    **Enforced Over**: GEN_COMMIT_CAP_OPR_TMPS
    ...
    if check_if_boundary_type_and_first_timepoint(
        ...
        return mod.GenCommitCap_CCS_Shutdown_MW[g, tmp] \
            >= prev_tmp_commit_capacity - mod.Commit_Capacity_CCS_MW[g, tmp]
...
def min_up_time_constraint_rule(mod, g, tmp):
    ...
    **Enforced Over**: GEN_COMMIT_CAP_OPR_TMPS
    ...
    relevant_tmps, relevant_linked_timepoints = determine_relevant_timepoints(
        mod, g, tmp, mod.gen_commit_cap_ccs_min_up_time_hours[g]
    )
    ...
    # started up in the relevant timepoints
        ...
        capacity_turned_on_min_up_time_or_less_hours_ago = \
            sum(mod.GenCommitCap_CCS_Startup_MW[g, tp] for tp in relevant_tmps) \
            + sum(mod.gen_commit_cap_ccs_linked_startup[g, ltp]
...
def min_down_time_constraint_rule(mod, g, tmp):
    ...
    **Enforced Over**: GEN_COMMIT_CAP_OPR_TMPS
    ...
    relevant_tmps, relevant_linked_timepoints = determine_relevant_timepoints(
        mod, g, tmp, mod.gen_commit_cap_ccs_min_down_time_hours[g]
    )

    capacity_turned_off_min_down_time_or_less_hours_ago = \
        sum(mod.GenCommitCap_CCS_Shutdown_MW[g, tp] for tp in relevant_tmps) \
        + sum(mod.gen_commit_cap_ccs_linked_shutdown[g, ltp]
              for ltp in relevant_linked_timepoints)
...
def ccs_efficiency_rule(mod, g):
    return mod.gen_commit_cap_ccs_ccs_efficiency[g]
...
def variable_om_cost_rule(mod, g, tmp):
    ...
    return mod.GenCommitCap_CCS_Provide_Power_MW[g, tmp] \
        * mod.variable_om_cost_per_mwh[g] \
        + mod.Gen_Capacity_CCS_Tonne[g,tmp] \
        * mod.gen_commit_cap_ccs_variable_ccs_om_cost_per_tonne[g]
...
def fix_commitment(mod, g, tmp):
    ...
    mod.Commit_Capacity_CCS_MW[g, tmp].fixed = True
...
def load_model_data(
    ...
    load_optype_model_data(
        ...
        stage=stage, op_type="gen_commit_cap_ccs"
    )
    '''
    ...
    linked_inputs_filename = os.path.join(
            ...
            "gen_commit_cap_ccs_linked_timepoint_params.tab"
        )
    if os.path.exists(linked_inputs_filename):
        data_portal.load(
            filename=linked_inputs_filename,
            index=mod.GEN_COMMIT_CAP_CCS_LINKED_TMPS,
            param=(
                mod.gen_commit_cap_ccs_linked_commit_capacity,
                mod.gen_commit_cap_ccs_linked_power,
                mod.gen_commit_cap_ccs_linked_upwards_reserves,
                mod.gen_commit_cap_ccs_linked_downwards_reserves,
                mod.gen_commit_cap_ccs_linked_startup,
                mod.gen_commit_cap_ccs_linked_shutdown
            )
        )
    else:
        pass
...
def export_results(
    ...
    with open(os.path.join(scenario_directory, str(subproblem), str(stage), "results",
                           "dispatch_capacity_commit_ccs.csv"),
              "w", newline="") as f:
        writer = csv.writer(f)
        writer.writerow(["project", "period", "balancing_type_project",
                         ...
                         "gross_power_mw",
                         "auxiliary_consumption_mw","ccs_power_mw", "net_power_mw",
                         "committed_mw","carbon_ccs_tonne", "committed_units"
                         ])

        for (p, tmp) \
                in mod. \
                GEN_COMMIT_CAP_CCS_OPR_TMPS:
            writer.writerow([
                p,
                mod.period[tmp],
                ...
                value(mod.GenCommitCap_CCS_Provide_Power_MW[p, tmp]),
                value(mod.GenCommitCap_CCS_Auxiliary_Consumption_MW[p, tmp]),
                value(mod.Gen_Capacity_CCS_Tonne[p,tmp]) * value (mod.gen_commit_cap_ccs_ccs_mwh_per_tonne[p]),
                value(mod.GenCommitCap_CCS_Provide_Power_MW[p, tmp]) -
                value(mod.GenCommitCap_CCS_Auxiliary_Consumption_MW[p, tmp])-
                value(mod.Gen_Capacity_CCS_Tonne[p,tmp]) * value (mod.gen_commit_cap_ccs_ccs_mwh_per_tonne[p]) ,
                value(mod.Commit_Capacity_CCS_MW[p, tmp]),
                value(mod.Gen_Capacity_CCS_Tonne[p,tmp]),
                value(mod.Commit_Capacity_CCS_MW[p, tmp]) /
                mod.gen_commit_cap_ccs_unit_size_mw[p]
            ])
    ...
    if tmps_to_link:
        ...
        with open(os.path.join(
            ...
            for (p, tmp) in sorted(mod.GEN_COMMIT_CAP_CCS_OPR_TMPS):
                if tmp in tmps_to_link:
                    writer.writerow([
                        p,
                        tmp_linked_tmp_dict[tmp],
                        max(value(mod.Commit_Capacity_CCS_MW[p, tmp]), 0),
                        max(value(mod.GenCommitCap_CCS_Provide_Power_MW[p, tmp]),
                            0),
                        max(value(mod.GenCommitCap_CCS_Upwards_Reserves_MW[p, tmp]
                                  ), 0),
                        max(value(mod.GenCommitCap_CCS_Downwards_Reserves_MW[
                                      p, tmp]), 0),
                        max(value(mod.GenCommitCap_CCS_Startup_MW[p, tmp]), 0),
                        max(value(mod.GenCommitCap_CCS_Shutdown_MW[p, tmp]), 0)
                    ])
...
def import_model_results_to_database(
    ...
    update_dispatch_results_table(
        ...
        results_file="dispatch_capacity_commit_ccs.csv"
    )
...
def validate_inputs(scenario_id, subscenarios, subproblem, stage, conn):
    ...
    validate_opchars(scenario_id, subscenarios, subproblem, stage, conn, "gen_commit_cap_ccs")
