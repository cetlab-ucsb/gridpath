...
def spec_determine_inputs(
    ...
    spec_ccs_fixed_cost_per_tonne_yr_dict=dict()
    ...
    for row in zip(df["project"],
                   ...
                   df["ccs_fixed_cost_per_tonne_yr"]):
        if row[0] in project_list:
            ...
            spec_fixed_cost_per_mwh_yr_dict[(row[0], row[1])] = \
                float(row[9])
            spec_ccs_fixed_cost_per_tonne_yr_dict[(row[0]), row[1]] = \
                float(row[10])
        else:
            pass
    ...
    main_dict["ccs_fixed_cost_per_tonne_yr"] = spec_ccs_fixed_cost_per_tonne_yr_dict
